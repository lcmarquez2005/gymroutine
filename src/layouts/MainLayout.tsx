...
export const MainLayout: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen bg-slate-50 flex justify-center ${showBottomNav ? 'pb-20' : ''}`}>
      <div className="w-full max-w-md relative flex flex-col">
        ...
        {showBottomNav && (
          <div className="fixed bottom-0 w-full max-w-md bg-white/80 backdrop-blur-xl border-t border-slate-200 px-6 py-3 flex justify-between items-center rounded-t-3xl shadow-[0_-5px_20px_-5px_rgba(0,0,0,0.05)] z-50">
            ...
            <Link to="/exercises" className={`flex flex-col items-center transition-colors duration-200 ${isActive('/exercises') ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>
              <BookOpen size={24} className={isActive('/exercises') ? 'fill-blue-50' : ''} />
